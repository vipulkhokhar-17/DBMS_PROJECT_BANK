...
public class UpdateLoanStatusController {
    ...
    private void handleUpdate() {
        ...
        if (loanNumber.isEmpty() || status == null) {
            statusLabel.setText("All fields are required.");
            statusLabel.setStyle("-fx-text-fill: #ff5252;");
            return;
        }

        boolean success = staffService.updateLoanStatus(loanNumber, status);
        if (success) {
            showSuccessAnimation("Loan status updated successfully.");
            loanNumberField.clear();
            statusCombo.getSelectionModel().clearSelection();
        } else {
            statusLabel.setText("Failed to update loan status.");
            statusLabel.setStyle("-fx-text-fill: #ff5252;");
        }
    }
